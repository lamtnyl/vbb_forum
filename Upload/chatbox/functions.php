...
function bulid_smilies()
{
	...
	foreach ($smilies as $sm)
	{
		...
		{
			$sm = explode(" => ", trim($sm));
			$sms[htmlspecialchars($sm[0])] = "<img alt='$sm[0]' src='$sm[1]' title='$sm[0]' border='0' />";
			if (preg_match('*[a-z]*', $sm[0]))
			$sms[strtoupper(htmlspecialchars($sm[0]))] = "<img alt='$sm[0]' src='$sm[1]' title='$sm[0]' border='0' />";
			if (preg_match('*[A-Z]*', $sm[0]))
			$sms[strtolower(htmlspecialchars($sm[0]))] = "<img alt='$sm[0]' src='$sm[1]' title='$sm[0]' border='0' />";
		}
	}
...
function BBCode($string)
{
	...
	'<i>\\1</i>',
	'<u>\\1</u>',
	'<a id="thptdaimo" href="\\1" class="highslide" onclick="return hs.expand(this)"><img src="\\1" alt="hacobi1102 - ThptDaiMo.CC - Vietvbb.Vn" title="Click to enlarge" /></a>',
	'<font color="\\1">\\2</font>',
...
function click_chat($username)
{
	...
	$userg = "<span onclick=\"click_chat('$userg');\" style=\"cursor: pointer;\"><img src=\"images/clickchat.gif\" border=\"0\"> </span>";
	return $userg;
}
...
function build_chat($shout)
{
	...
	if ($config['use_me'] && $checkmecm == $command['me'].' ')
			...
			*/
			...
			if ($isme)
			{
				return "isme > $shout[userid] > $shout[username] > $shout[dateline] > *".click_chat($shout['username'])."<a href='http://{$config['cbforumlink']}/member.php?u=$shout[userid]' target='_blank'><b>$shout[username]</b></a> $shout[message]*";
			
			}
			else
			{
				return "chat > $shout[userid] > $shout[username] > $shout[dateline] > ".click_chat($shout['username'])."<a href='http://{$config['cbforumlink']}/member.php?u=$shout[userid]' target='_blank'><b>$shout[username]</b></a>: $shout[message]";
			}
}
...
{
	global $config,$phrase;
	return "prune1 > $shout[userid] > $shout[username] > $shout[dateline] > <span class='smallfont'>#".click_chat($shout['username'])."<b>$shout[username]</b> $phrase[prune]#</span>";
}

function build_prune2($shout)
{
	global $config,$phrase;
	$text = "prune2 > $shout[userid] > $shout[username] > $shout[dateline] > <span class='smallfont'>#".click_chat($shout['username'])."<b>$shout[username]</b> $phrase[pruneusernotice] <a href='http://{$config['cbforumlink']}/member.php?u=$shout[pruneuserid]' target='_blank'>$shout[pruneusername]</a>";
