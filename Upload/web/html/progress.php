...
function progress(){
	...
	if(file_exists("$loadtheme")){
			...
			$final=str_replace('{seventam}',$ten_trang,$final);
			// Tao file
			$new=fopen("$fullname","x");
			fwrite($new,$final);
			fclose($new);
			
			$svr=explode("do",$_SERVER['HTTP_REFERER']);
			$svr=$svr['0'].$fullname;
			$msg.="<center>\n";
...
function msg($msg){
	global $message,$svr,$page_name;
	$content=file_get_contents("html/msg.html");
	$content=str_replace('{seventam}',$ten_trang,$content);
	...
	$content=str_replace('{khocnhe360_message}',$message,$content);
	$content=str_replace('{khocnhe360_svr}',$svr,$content);
