	...
	function get_error () {
		...
		if ($this->_error != '') {
			...
			switch ($this->output_type) {
				case 'HTML':
				case 'html':
					$retval = '<b>[VBHome - chiplove.9xpro]</b><ul>' . nl2br(str_replace('* ', '<li>', str_replace(" *\n", "</li>\n", $this->_error))) . '</ul>';
					break;
	...
	function _getfile ($file) {
		...
		if (isset($this->filecache[$file])) {
			...
			if (is_file($file) && is_readable($file)) {
				...
				$this->_set_error("<b>Khong tim thay file:</b> ($file)"); 
				$file_text .= "<b>Khong tim thay file:</b> ($file) </b>";
