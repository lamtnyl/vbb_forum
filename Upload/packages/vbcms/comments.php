 ...
 * @version $Id: comments.php 59389 2012-02-21 01:11:26Z pmarsden $
 * @access public
 */
class vBCms_Comments
{
	...
	***/
	private static function getComments($nodeid, $userinfo, &$permissions, $allcomments, $associatedthreadid)
	{
		require_once DIR . '/vb/cache.php';
		$comments = vB_Cache::instance()->read(self::getStaticHash($nodeid));
		if ($comments AND !empty($comments))
		{
			return $comments;
		}
	...
	*********/
	...
	private static function showComments($nodeid, $userinfo, $pageno,
		$perpage, $target_url, $allcomments, $associatedthreadid)
	{
		require_once DIR . '/includes/functions_misc.php';
		...
		//get our results
		$results = self::getComments($nodeid, $userinfo, $permissions, $allcomments, $associatedthreadid);
		...
		$pagenav = construct_page_nav($pageno, $perpage, $record_count, '', '', 'comments', 'vbcms');
		...
		while ($post = vB::$vbulletin->db->fetch_array($posts))
		{
			...
			if (!$post['hascustomavatar'] AND !$post['avatarid'])
			{
				if ($post['profilepic'])
				{
					...
					{
						$post['avatarpath'] = vB::$vbulletin->options['profilepicurl'] . '/profilepic' . $post['userid'] . '_' . $post['profilepicrevision'] . '.gif';
					}
					else
					{
