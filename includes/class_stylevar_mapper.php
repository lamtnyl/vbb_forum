...
class SV_Mapping 
{
	...
	public function add_sv_preset($mapto, $value, $forced = true)
	{
		// Add a preset
		$this->mappings = true;
		$this->preset[$mapto] = array($value, $forced);
	}
	...
	function process()
	{
			...
			foreach($destinations AS $destination)
			{
				...
				if ($this->custom[$source_var])
				{
					foreach($this->custom[$source_var] AS $style => $source_data)
					{
						...
						if (!$merge AND empty($destination_data))
						{
		...
		foreach($this->preset AS $source => $value_data)
		{
			...
			if ($source_data)
			{
				...
				{
					list($value, $forced) = $value_data;
					if(empty($destination_data[$source_type]) OR $forced)
					{
						$this->processed = true;
						$destination_data[$source_type] = $value;
