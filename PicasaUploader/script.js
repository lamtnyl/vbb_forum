...
function renderUpload(wtm) {
    document.getElementById('flash_upload').innerHTML = '<embed type="application/x-shockwave-flash" quality="high" wmode="transparent" ' + 'pluginurl="http://www.macromedia.com/go/getflashplayer" pluginspage="http://www.macromedia.com/go/getflashplayer" src="upload.swf?' + (getWatermark() ? 'watermark=1' : 'watermark=0') + '&logo=' + getLogo() + '&resize=' + getResize() + '" width="75" height="25"></embed>';
...
function showcode(type) {
    ...
    var html = '';
    var re = /<div>.*?(https?:.*?)\.(jpg|png|bmp|gif|jpeg)<\/div>/ig;
...
function transfer(id) {
    ...
    var _wtm = document.getElementById('watermark').checked ? 'yes' : 'no';
    text = document.getElementById('listurl').value;
    var re = /.*?(\[IMG\])?(https?[^\[]*)(\[\/IMG\])?/ig;
    var m = re.exec(text);
    var html = "";
    while (m) {
       html += m[2] +"\n";
...
function transferUrl(url, transfer_id, total, _wtm) {
    ...
    transfer_id += 1;
    ajax.send('upload.php', 'url=' + encodeURI(url) + '&watermark=' + _wtm + '&logo=' + getLogo() + '&resize=' + getResize() + '&sid=' + Math.random());

    function check() {
        if (ajax.xmlHttp.readyState == 4 && ajax.xmlHttp.status == 200) {
            stopLoading();
            dv = document.createElement("div");
            dv.innerHTML = transfer_id + '/' + (total-1) + ' - ' + ajax.xmlHttp.responseText;
